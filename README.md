...
***hint***: There are two response types supported with this plugin: `code id_token` or `code`. Use the ResponseType directive in appSettings to select the response type. When the response type is `code id_token` (the `Hybrid flow`), plugin  gets a `code` and `id_token` in the Authentication Response directly but uses the returned `code` to get an `access_token` from the Token endpoint. The `code` response type, authorization code flow, returns only a `code` from the Authorization endpoint. The `id_token` and `access_token` are then retrieved from the Token endpoint using the `code`.
